...
Lwt is a library for creating and manipulating _promises_.
...
A promise that doesn't hold anything is said to be _pending_.
When a promise that holds nothing changes to hold something it is said to _resolve_ or to _be resolved_.
In particular, a promise that resolves to hold a value is said to be _fulfilled_ whereas a promise that resolves to hold an exception is said to be _rejected_.
...
`task ()` evaluates immediately to a pair `(p, r)` where `p` is a pending promise and `r` is its associated _resolver_.
...
It is _possible_ to use `fail` instead, but, as mentioned earlier, `raise` is _preferred_.
